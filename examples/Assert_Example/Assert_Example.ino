 ...
 * @date      Feb 4 2026
 *
 * @brief     Simple demonstration of using RML_COMM_ASSERT with the Remal logger.
 ...
 *
 *            NOTE: #define RML_COMM_ASSERT_ENABLE must be added to the top of the file
 *            for asserts to work, more specifically before including Remal_CommonUtils.h.
 *            If RML_COMM_ASSERT_ENABLE is not defined, all assertions will be compiled out,
 ...
 *            cleanup, save failure data, or blink an LED to indicate the error.
*/
#define RML_COMM_ASSERT_ENABLE				// <---- Must be defined before including Remal_CommonUtils.h for asserts to work
 ...
 *        Use this to save failure data, blink an LED, or perform cleanup.
 */
void MyAssertCallback(const char* FileName, uint32_t LineNumber, const char* Expression)
{
	...
	RML_COMM_Log_Msg("Callback", e_WARNING, "Assert callback triggered! You could save failure data here, blink an LED to indicate the error or perform cleanup before the program halts.");
}
...
void setup()
{
	...
	int TestValue = 10;
	RML_COMM_Log_Msg("Test", e_INFO, "Test value is: %d", TestValue);

	// Assert that TestValue should never exceed 5 (this will intentionally fail)
	RML_COMM_ASSERT(TestValue <= 5);

	// This message will never be logged due to the assert above
	RML_COMM_Log_Msg("Test", e_INFO, "If you see this message, the assert is disabled. Enable by defining RML_COMM_ASSERT_ENABLE before including Remal_CommonUtils.h");
}
