 ...
 *            using USB CDC with various log levels and format specifiers.
*/
...
void setup()
{
    ...
    if( RML_COMM_Logger_Init(e_USB) != 0 )
    {
        // Logger initialization failed, blink LED RED to indicate error
        while(1)
        {
            RML_COMM_LED_SetColor(Shabakah_LED_1, 255, 0, 0);   // RED
            RML_COMM_LED_SetColor(Shabakah_LED_2, 255, 0, 0);   // RED
            delay(500);
            RML_COMM_LED_SetColor(Shabakah_LED_1, 0, 0, 0);     // OFF
            RML_COMM_LED_SetColor(Shabakah_LED_2, 0, 0, 0);     // OFF
            delay(500);
        }
    }
